...
import java.util.*;
  ...
  class CodeGen {
	...
	public static void startForLoop(Entry var) {
    // Generates set up for for loop
      //this is the point we want to branch to 
      emit(PVM.lda); emit(var.offset); // put address of identifier on stack
      emit(PVM.ffs); // use our special opcode to put the current expression on top of stack,
                      // and decrement the count of expressions on stack.
      emit(PVM.sto); // store in the address of identifier, we are now ready to enter the statement
    } // CodeGen.startForLoop

    public static void endForLoop(Label loopStart, Label loopExit) {
    // Generates end check for for loop
      emit(PVM.cpy); //copy the top value of stack
      emit(PVM.bze); emit(loopExit.address()); //exit the loop when finished expressions 
      emit(PVM.brn); emit(loopStart.address()); //otherwise return to the beginning
    } // CodeGen.endForLoop

    public static void popMultiple(int num) {
    // pops num entries off the stack
      emit(PVM.popM);  emit(num); 
    }
