      ...
      do {
        ...
        switch (cpu.ir) {         // execute
          ...
          case PVM.heap:          // heap dump (debugging)
            ...
            break;
		  case PVM.ffs: //Fetch from stack: puts adress of identifier and next expression on stack 
            sos = pop(); // adress of the identifier, will become the sos
            int count = pop(); // expression count
            tos = mem[cpu.sp + count - 1]; //fetch the current expression, will become tos
            push(count - 1); //decrement the expression count
            push(sos); // put address of identifier on stack
            push(tos); // put expression on stack
            break;
          case PVM.popM: //pops multiple values from stack
            int num = next();
            cpu.sp += num;
            break;
          case PVM.cpy: //copy tos
            tos = pop();
            push(tos);
            push(tos);
            break;
...
//        data.close();
    ...
    // Initializes stack machine
	  ...
	  mnemonics[PVM.ffs]	  = "FFS";
	  mnemonics[PVM.cpy]      = "CPY";
	  mnemonics[PVM.popM]	  = "POPM";
    } // PVM.init
